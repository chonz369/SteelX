...
[CreateAssetMenu(fileName = "Stand", menuName = "SteelX/Animation/AnimGraph/Stand")]
public class AnimGraph_Stand : AnimGraphAsset
{
    ...
    class CharacterAnimGraph_3PStand : IAnimGraphInstance, IGraphState
    {
        public CharacterAnimGraph_3PStand(EntityManager entityManager, Entity owner, PlayableGraph graph, Entity animStateOwner, AnimGraph_Stand template) {
            ...
            GameDebug.Assert(entityManager.HasComponent<MechSkeleton>(owner), "Owner has no Skeleton component");
            var skeleton = entityManager.GetComponentObject<MechSkeleton>(owner);
            ...
            var leftToes = skeleton.leftToeBone;
            var rightToes = skeleton.rightToeBone;
            ...
            m_animIdle.SetApplyFootIK(false);
        ...
        public void UpdatePresentationState(bool firstUpdate, GameTime time, float deltaTime) {
            ...
            if (animState.turnDirection == 0) {
                ...
                if (absAimYawLocal > m_template.aimTurnLocalThreshold) // TODO: Document why we need local vs non local turn tolerances?
                {
